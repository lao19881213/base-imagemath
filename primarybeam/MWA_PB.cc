...
namespace askap {
    namespace imagemath {
            ...
            PrimaryBeam::ShPtr MWA_PB::createDefaultPrimaryBeam() {
               ...
               ptr->setLatitude(-26.703319 * casacore::C::pi/180.);
               ptr->setLongitude(116.67081 * casacore::C::pi/180.);
            ...
            PrimaryBeam::ShPtr MWA_PB::createPrimaryBeam(const LOFAR::ParameterSet &parset)
            {
               ...
               ptr->setLatitude(parset.getDouble("latitude",-26.703319 * casacore::C::pi/180.));
               ptr->setLongitude(parset.getDouble("longitude",116.67081 * casacore::C::pi/180.));
            ...
            casacore::Matrix<casacore::Complex> MWA_PB::getJonesAtOffset(double az, double za, double frequency) {
                ...
                ASKAPCHECK(za <= casacore::C::pi/2.0, "za must not be larger than pi/2");

                const double lambda = casacore::C::c/frequency;
                ...
                const double mult = 2. * casacore::C::pi / lambda;
                ...
                for ( int i = 0; i < numDipoleColumns; i++ ) {
                    for ( int j = 0; j < numDipoleRows; j++ ) {
                        ...
                        //dipl_z = 0.0;
                        double phase = mult*(dipl_e*proj_e + dipl_n*proj_n - delays[k]*435.0e-12*casacore::C::c);
                ...
                float groundPlane = 2.*sin(2.*casacore::C::pi*DipoleHeight/lambda*cos(za));
                // probably want this scaling as an optional, user-defined option
                groundPlane /= 2.*sin(2.*casacore::C::pi*DipoleHeight/lambda);
